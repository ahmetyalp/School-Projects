...
def assignTask(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            conn = psycopg2.connect("dbname=database.db user=postgres password=1234")
            c = conn.cursor()
            t = (form.cleaned_data['pname'] , request.session.get('user') , t)
            ...
            x = c.fetchall()[0]
            t = (form.cleaned_data['id'] , )
            c.execute("SELECT startdate, finishdate FROM Task WHERE EXISTS (SELECT * ET WHERE tid = id AND eid = %s)" , t)
            y = c.fetchall()
            for q in y:
